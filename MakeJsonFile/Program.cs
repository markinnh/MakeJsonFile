...
if (!CheckIfContentsNeedsUpdating(inpath, manifest.ColorsLastUpdated))
{
    Console.WriteLine("Color Masters are up to date, no need to update the json file.");
    return;
}
else
{
...
if (colorsUpdated)
    Manifest.UpdateManifest(Path.Combine("C:\\Users\\markn\\source\\repos\\MakeJsonFile\\MakeJsonFile", "SharedFilamentData", "Manifest.json"), colorUpdated: colorsUpdated, vendorsUpdated: false, materialUpdated: false);
